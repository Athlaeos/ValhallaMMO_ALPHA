...
public class PartyCommand implements TabExecutor {
	...
	public boolean onCommand(CommandSender s, org.bukkit.command.Command cmd, String name, String[] args) {
		...
		if (args.length > 0){
			if (args[0].equalsIgnoreCase("create")){
				...
				if (args.length > 1) {
					String partyName = String.join(" ", Arrays.copyOfRange(args, 1, args.length));
					PartyManager.ErrorStatus inviteStatus = m.acceptInvite(sender, partyName);
					if (inviteStatus != null) {
						inviteStatus.sendErrorMessage(sender);
						return true;
					}
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
		if (args.length == 1){
			List<String> suggestions = new ArrayList<>(Arrays.asList("toggleinvites", "join", "leave", "shareitem", "info"));
			if (sender.hasPermission("valhalla.createparty")){
				suggestions = Arrays.asList("toggleinvites", "disband", "create", "invite", "join", "leave", "kick", "shareitem", "info", "setdescription", "transferleader", "setrank", "toggleopenstatus", "toggleexpsharing", "toggleitemsharing");
			}
			return suggestions;
