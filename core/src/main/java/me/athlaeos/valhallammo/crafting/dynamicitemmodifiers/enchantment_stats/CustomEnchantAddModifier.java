...
public class CustomEnchantAddModifier extends DuoArgDynamicItemModifier {
    ...
    public ItemStack processItem(Player crafter, ItemStack outputItem, int timesExecuted) {
        ...
        if (CustomEnchantmentManager.getInstance().getCustomEnchant(outputItem, enchantment) != null) {
            return null;
        }
        ...
        if (strength2 > 0){
            if (crafter == null) return null;
            int enchantingSkill = (int) ((strength2 / 100) * Math.floor(AccumulativeStatManager.getInstance().getEntityStatsIncludingCache("ENCHANTING_QUALITY_GENERAL", crafter, 2000, this.use) + AccumulativeStatManager.getInstance().getStats("ENCHANTING_QUALITY_CUSTOM", crafter, this.use)));
