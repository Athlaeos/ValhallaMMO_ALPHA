...
public class ChangeItemToDictionaryItemModifier extends DynamicItemModifier implements Cloneable {

    public ChangeItemToDictionaryItemModifier(String name, double strength, ModifierPriority priority) {
        ...
        this.description = Utils.chat("&7Sets an item to one of the indexed items in the item index, accessible with /val itemindex. " +
                "This changes the item completely, so any previous properties are lost. Keep good track of the index, as changing an item " +
                "in it can change the outcome of this recipe. ");
        this.displayName = Utils.chat("&a&lSet To Indexed Item");
    ...
    public ItemStack processItem(Player crafter, ItemStack outputItem) {
        return ItemDictionaryManager.getInstance().getItemDictionary().get((int) strength);
    }
