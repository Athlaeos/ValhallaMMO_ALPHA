...
public class DynamicArmorModifier extends DuoArgDynamicItemModifier {
    public DynamicArmorModifier(String name, double strength, double strength2, ModifierPriority priority) {
        ...
        this.description = Utils.chat("&7Updates the item's armor value if it has one. The strength of the modifier " +
                "represents the % of the item's quality rating used in determining its armor rating. Example: " +
                "if an item has a quality rating of 150, setting a strength of 50% will update the " +
                "item's armor rating to if it had a rating of 75, and 200% results in a rating of 300. " +
                "The second strength determines the minimum value of the attribute compared to its default. If this value " +
                "is 0.8 for example, a base armor of 10 cannot be reduced to anything below 8 and will always be 8" +
                " or higher. This is to make sure low skill levels don't suffer too much of a penalty when crafting such recipes");
        this.displayName = Utils.chat("&c&lUpdate Armor");
    ...
    public ItemStack processItem(Player crafter, ItemStack outputItem) {
        ...
        SmithingItemTreatmentManager.getInstance().applyAttributeScaling(outputItem, finalQuality, "GENERIC_ARMOR", strength2);
        return outputItem;
    }

    @Override
    public String toString() {
        return Utils.chat(String.format("&7Setting the item's armor rating to be &e%s%%&7 efficient with its quality score. " +
