...
public class DynamicHealthBoostModifier extends DuoArgDynamicItemModifier {
    public DynamicHealthBoostModifier(String name, double strength, double strength2, ModifierPriority priority) {
        ...
        this.description = Utils.chat("&7Updates the item's health boost value if it has one. The strength of the modifier " +
                "represents the % of the item's quality rating used in determining its health boost. Example: " +
                "if an item has a quality rating of 150, setting a strength of 50% will update the " +
                "item's health boost to if it had a rating of 75, and 200% results in a rating of 300. " +
                "The second strength determines the minimum value of the attribute compared to its default. If this value " +
                "is 0.8 for example, a base health boost of 10 cannot be reduced to anything below 8 and will always be 8" +
                " or higher. This is to make sure low skill levels don't suffer too much of a penalty when crafting such recipes");
        this.displayName = Utils.chat("&c&lUpdate Health Boost");
    ...
    public ItemStack processItem(Player crafter, ItemStack outputItem) {
        ...
        SmithingItemTreatmentManager.getInstance().applyAttributeScaling(outputItem, finalQuality, "GENERIC_MAX_HEALTH", strength2);
        return outputItem;
    }

    @Override
    public String toString() {
        return Utils.chat(String.format("&7Setting the item's health boost to be &e%s%%&7 efficient with its quality score. " +
