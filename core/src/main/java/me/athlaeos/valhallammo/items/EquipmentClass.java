...
public enum EquipmentClass {
    ...
    private final static NamespacedKey equipmentClassKey = new NamespacedKey(ValhallaMMO.getPlugin(), "equipment_class");
    ...
    public static EquipmentClass getClass(ItemStack item) {
        ItemMeta meta = item.getItemMeta();
        if (meta == null) return null;
        if (ValhallaMMO.isTrinketsHooked()){
            if (TrinketsManager.getInstance().getTrinketType(item) != null) return TRINKET;
        }
    ...
    public static boolean isArmor(ItemStack m) {
        EquipmentClass equipmentClass = getClass(m);
