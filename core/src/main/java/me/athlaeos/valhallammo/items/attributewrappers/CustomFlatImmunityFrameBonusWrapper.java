...
public class CustomFlatImmunityFrameBonusWrapper extends AttributeWrapper{
    private final String displayLore = Utils.chat(TranslationManager.getInstance().getTranslation("translation_immunityframeflatbonus"));

    public CustomFlatImmunityFrameBonusWrapper(double amount, AttributeModifier.Operation operation, EquipmentSlot slot) {
        super(amount, operation, slot);
        this.attribute = "CUSTOM_FLAT_IMMUNITY_FRAME_BONUS";
    ...
    private void updateLore(ItemMeta meta){
        ...
        if (!displayLore.equals("")){
            String stringValue = String.format((value >= 0 ? "+" : "") + "%d", value);
