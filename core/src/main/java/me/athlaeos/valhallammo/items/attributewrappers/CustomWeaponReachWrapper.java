...
public class CustomWeaponReachWrapper extends AttributeWrapper{
    private final String displayLore = Utils.chat(TranslationManager.getInstance().getTranslation("translation_weaponreach"));

    public CustomWeaponReachWrapper(double amount, AttributeModifier.Operation operation, EquipmentSlot slot) {
        super(amount, operation, slot);
        this.attribute = "CUSTOM_WEAPON_REACH";
    ...
    private void updateLore(ItemMeta meta){
        ...
        if (!displayLore.equals("")){
            String stringValue = String.format((value >= 0 ? "+" : "") + "%.2f", value);
