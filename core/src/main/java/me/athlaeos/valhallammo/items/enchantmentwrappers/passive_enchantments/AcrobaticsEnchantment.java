...
public class AcrobaticsEnchantment extends EnchantmentWrapper {
    private final String translation = Utils.chat(TranslationManager.getInstance().getTranslation("enchantment_acrobatics"));

    public AcrobaticsEnchantment(double amount) {
        super(amount);
        this.attribute = "ACROBATICS";
    ...
    private void updateLore(ItemMeta meta){
        ...
        if (!translation.equals("")){
            String strength = ((amplifier < 0) ? "" : "+") + String.format("%d", (int) Math.floor(amplifier*100));
