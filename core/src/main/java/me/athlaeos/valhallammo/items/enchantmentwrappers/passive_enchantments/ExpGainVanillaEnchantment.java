...
public class ExpGainVanillaEnchantment extends EnchantmentWrapper {
    private final String rangedDamageTranslation = Utils.chat(TranslationManager.getInstance().getTranslation("enchantment_exp_vanilla_gain"));

    public ExpGainVanillaEnchantment(double amount) {
        super(amount);
        this.attribute = "EXP_GAIN_VANILLA";
    ...
    private void updateLore(ItemMeta meta){
        ...
        if (!rangedDamageTranslation.equals("")){
            String bowStrength = ((bow_strength < 0) ? "" : "+") + String.format("%d", (int) Math.round(bow_strength * 100));
