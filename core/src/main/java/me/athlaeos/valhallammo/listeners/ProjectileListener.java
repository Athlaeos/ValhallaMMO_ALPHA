    ...
    public void onBowShoot(EntityShootBowEvent e){
        ...
        if (!e.isCancelled()){
            if (e.getProjectile() instanceof AbstractArrow){
                ...
                ArcherySkill.ChargedShotData data = ArcherySkill.getChargedShotUsers().get(shooter.getUniqueId());
                if (data != null){
                    if (data.isFullVelocity()) e.getProjectile().setVelocity(e.getProjectile().getVelocity().normalize().multiply(3));
                    projectileSpeed += ArcherySkill.getChargedShotUsers().get(shooter.getUniqueId()).getVelocityBonus();
                }
