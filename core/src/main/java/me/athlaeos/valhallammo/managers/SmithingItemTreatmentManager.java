...
public class SmithingItemTreatmentManager {
    private static SmithingItemTreatmentManager manager = null;
    ...
    // scale differently than a diamond chestplate depending on the quality, because these items have defined
    ...
    // the item's material.
    // Also, GENERIC_MAX_HEALTH is no longer a global attribute. There are currently no global attributes defined
    // by default.
    ...
    /**
     * @param i the ItemStack to get its tool id from
     * @return the tool id the given ItemStack has, or -1 if the item or its meta is null, or if the
     * item doesn't have the appropriate NamespacedKey
     */
    public int getItemsToolId(ItemStack i){
        if (Utils.isItemEmptyOrNull(i)) return -1;
        if (i.getItemMeta() == null) return -1;
        ItemMeta meta = i.getItemMeta();
        if (meta.getPersistentDataContainer().has(key_tool_id, PersistentDataType.INTEGER)){
            return meta.getPersistentDataContainer().get(key_tool_id, PersistentDataType.INTEGER);
        }
        return -1;
    }
    ...
    public boolean isItemCustom(ItemStack i){
        ...
        if (i.getItemMeta().getPersistentDataContainer().has(AlchemyPotionTreatmentManager.getInstance().getKey_brewing_treatment(), PersistentDataType.STRING)){
            return true;
        }
        if (i.getItemMeta().getPersistentDataContainer().has(AlchemyPotionTreatmentManager.getInstance().getKey_quality_general(), PersistentDataType.INTEGER)){
            return true;
        }
