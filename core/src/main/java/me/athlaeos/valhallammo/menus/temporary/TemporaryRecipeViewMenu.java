...
public class TemporaryRecipeViewMenu extends Menu {
    ...
    private final ItemStack viewCustomSmithingRecipesButton = Utils.createItemStack(
            ...
            Utils.separateStringIntoLines(Utils.chat("&7Crafting or improving items using the smithing table. "), 40));
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (clickedItem != null) {
            if (clickedItem.equals(viewCustomCraftRecipesButton)) {
                new ViewCraftRecipeMenu(playerMenuUtility).open();
                return;
            } else if (clickedItem.equals(viewCustomImproveRecipesButton)){
                new ViewTinkerRecipeMenu(playerMenuUtility).open();
                return;
            } else if (clickedItem.equals(viewShapedRecipesButton)){
                new ViewShapedRecipeMenu(playerMenuUtility).open();
                return;
            } else if (clickedItem.equals(viewCampfireRecipesButton)){
                new ViewCookingRecipeMenu(playerMenuUtility, ViewCookingRecipeMenu.RecipeType.CAMPFIRE).open();
                return;
            } else if (clickedItem.equals(viewFurnaceRecipesButton)){
                new ViewCookingRecipeMenu(playerMenuUtility, ViewCookingRecipeMenu.RecipeType.FURNACE).open();
                return;
            } else if (clickedItem.equals(viewCustomSmithingRecipesButton)){
                new ViewSmithingRecipeMenu(playerMenuUtility).open();
                return;
            } else if (clickedItem.equals(viewCauldronRecipesButton)){
                new ViewCauldronRecipeMenu(playerMenuUtility).open();
                return;
            } else if (clickedItem.equals(viewBlastFurnaceRecipesButton)){
                new ViewCookingRecipeMenu(playerMenuUtility, ViewCookingRecipeMenu.RecipeType.BLAST_FURNACE).open();
                return;
            } else if (clickedItem.equals(viewSmokerRecipesButton)){
                new ViewCookingRecipeMenu(playerMenuUtility, ViewCookingRecipeMenu.RecipeType.SMOKER).open();
                return;
            } else if (clickedItem.equals(viewBrewingRecipesButton)){
                new ViewBrewingRecipeMenu(playerMenuUtility).open();
                return;
            } else if (clickedItem.equals(viewCustomClassImproveRecipesButton)){
                new ViewClassTinkerRecipeMenu(playerMenuUtility).open();
                return;
            }
        }
        setMenuItems();
