...
public class FarmingFishingRewardTier extends Placeholder {
    public FarmingFishingRewardTier(String placeholder) {
        super(placeholder);
    }
    ...
    public String parse(String s, Player p) {
        ...
        if (profile != null){
            if (profile instanceof FarmingProfile){
                float tier = ((FarmingProfile) profile).getFishingRewardTier();
                return s.replace(this.placeholder, String.format("%,.1f", tier));
            }
        }
        return s;
    }
