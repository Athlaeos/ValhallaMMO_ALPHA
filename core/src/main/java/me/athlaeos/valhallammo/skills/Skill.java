...
public abstract class Skill {
    ...
    public double expForlevel(int level){
        if (active && level <= max_level){
            return Utils.round(Utils.eval(expCurve.replace("%level%", "" + level)), 4);
        } else {
            return -1;
        }
    }
     ...
     */
    public void updateLevelUpConditions(Player p, boolean silent, PlayerSkillExperienceGainEvent.ExperienceGainReason reason){
        Profile profile = ProfileManager.getManager().getProfile(p, this.type);
        ...
        if (profile != null){
            ...
            while (remainingEXP >= nextLevelEXP){
                if (nextLevel <= max_level){
                    levelPlayerUp(p, profile, silent, reason);
                    nextLevel += 1;
...
//                            }
        ...
        if (!event.isCancelled()){
            ...
            if (profile != null){
                ...
                if (profile.getExp() >= expForlevel(profile.getLevel() + 1)){
                    updateLevelUpConditions(p, silent, reason);
                }
            }
        }
    }
     ...
     */
    public void levelPlayerUp(Player p, Profile profile, boolean silent, PlayerSkillExperienceGainEvent.ExperienceGainReason reason){
        if (!active) return;
        if (profile == null) profile = ProfileManager.getManager().newProfile(p, this.type);
        if (profile != null){
            ...
            if (!this.getType().equals("ACCOUNT")){
                ...
                if (accountSkill != null) {
                    if (accountSkill instanceof AccountSkill){
                        ((AccountSkill) accountSkill).addAccountEXP(p, ((AccountSkill) accountSkill).getExpPerLevelUp(), reason);
                    }
                }
            }
