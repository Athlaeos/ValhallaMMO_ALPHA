...
public class ArmorToughnessBonus extends AccumulativeStatSource {
    @Override
            ...
            if (p instanceof Player){
                ...
                if (profile != null){
                    if (profile instanceof AccountProfile){
                        amount += ((AccountProfile) profile).getToughnessBonus();
                    }
                }
            }
            ...
            if (ValhallaMMO.isTrinketsHooked()){
                ...
                for (ItemStack i : entityProperties.getIterable(false)){
                    if (EquipmentClass.getClass(i) == EquipmentClass.TRINKET) {
                        AttributeWrapper attributeWrapper = ItemAttributesManager.getInstance().getAnyAttributeWrapper(i, "GENERIC_ARMOR_TOUGHNESS");
