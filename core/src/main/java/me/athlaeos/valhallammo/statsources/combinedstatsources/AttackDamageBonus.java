...
public class AttackDamageBonus extends AccumulativeStatSource {
    @Override
            ...
            if (ValhallaMMO.isTrinketsHooked()){
                ...
                for (ItemStack i : entityProperties.getIterable(false)){
                    if (EquipmentClass.getClass(i) == EquipmentClass.TRINKET) {
                        AttributeWrapper attributeWrapper = ItemAttributesManager.getInstance().getAnyAttributeWrapper(i, "GENERIC_ATTACK_DAMAGE");
