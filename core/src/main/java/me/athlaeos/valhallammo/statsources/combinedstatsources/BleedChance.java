...
public class BleedChance extends EvEAccumulativeStatSource {
    @Override
                    ...
                    if (WeaponType.getWeaponType(hand) == WeaponType.LIGHT) {
                        ...
                        if (profile != null){
                            if (profile instanceof LightWeaponsProfile){
                                amount += ((LightWeaponsProfile) profile).getBleedChance();
                            }
                        }
                        ...
                        if (profile != null){
                            if (profile instanceof HeavyWeaponsProfile){
                                amount += ((HeavyWeaponsProfile) profile).getBleedChance();
                            }
                        }
                    }
                }
                if (EquipmentClass.getClass(hand) != EquipmentClass.TRINKET && !EquipmentClass.isArmor(hand)) {
                    AttributeWrapper wrapper = ItemAttributesManager.getInstance().getAttributeWrapper(hand, "CUSTOM_BLEED_CHANCE");
                    if (wrapper != null) amount += wrapper.getAmount();
                }
            }
            for (ItemStack i : entityProperties.getIterable(false)){
                AttributeWrapper wrapper = ItemAttributesManager.getInstance().getAttributeWrapper(i, "CUSTOM_BLEED_CHANCE");
                if (wrapper != null) amount += wrapper.getAmount();
            }
            amount += ItemUtils.combinedCustomEnchantAmplifier((LivingEntity) offender, "BLEED_CHANCE");
        }
        return amount;
    }
}
