...
public class HeavyWeaponsCritDamage extends EvEAccumulativeStatSource {
    @Override
        ...
        if (offender instanceof LivingEntity){
            ...
            if (!Utils.isItemEmptyOrNull(hand)){
                if (offender instanceof Player){
                    if (WeaponType.getWeaponType(hand) == WeaponType.HEAVY) {
                        ...
                        if (profile != null){
                            if (profile instanceof HeavyWeaponsProfile){
                                amount += ((HeavyWeaponsProfile) profile).getCritDamageMultiplier() - 1;
                            }
                        }
                    }
                }
                if (EquipmentClass.getClass(hand) != EquipmentClass.TRINKET && !EquipmentClass.isArmor(hand)) {
                    AttributeWrapper wrapper = ItemAttributesManager.getInstance().getAttributeWrapper(hand, "CUSTOM_CRIT_DAMAGE");
                    if (wrapper != null) amount += wrapper.getAmount();
                }
            }
            for (ItemStack i : entityProperties.getIterable(false)){
                AttributeWrapper wrapper = ItemAttributesManager.getInstance().getAttributeWrapper(i, "CUSTOM_CRIT_DAMAGE");
                if (wrapper != null) amount += wrapper.getAmount();
            }
            amount += ItemUtils.combinedCustomEnchantAmplifier((LivingEntity) offender, "CRIT_DAMAGE");
        }
        return amount;
    }
}
