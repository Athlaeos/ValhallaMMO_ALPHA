...
public class Knockback extends EvEAccumulativeStatSource {
    @Override
    ...
    public double add(Entity entity, Entity offender, boolean use) {
        ...
        if (offender instanceof LivingEntity){
                    ...
                    if (WeaponType.getWeaponType(hand) == WeaponType.LIGHT) {
                        ...
                        if (profile != null){
                            if (profile instanceof LightWeaponsProfile){
                                amount += ((LightWeaponsProfile) profile).getKnockbackBonus();
                            }
                        }
                        ...
                        if (profile != null){
                            if (profile instanceof HeavyWeaponsProfile){
                                amount += ((HeavyWeaponsProfile) profile).getKnockbackBonus();
                            }
                        }
                    }
                }
            ...
            for (ItemStack i : entityProperties.getIterable(false)){
                AttributeWrapper wrapper = ItemAttributesManager.getInstance().getAttributeWrapper(i, "CUSTOM_KNOCKBACK");
                if (wrapper != null) amount += wrapper.getAmount();
            }

            PotionEffect activePotionEffect = PotionEffectManager.getInstance().getPotionEffect(offender, "KNOCKBACK_BONUS");
