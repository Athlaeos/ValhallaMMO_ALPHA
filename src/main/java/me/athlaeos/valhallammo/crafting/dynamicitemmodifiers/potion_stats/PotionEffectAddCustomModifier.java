...
public class PotionEffectAddVanillaModifier extends DuoArgDynamicItemModifier {
    ...
    public PotionEffectAddVanillaModifier(String name, double duration_ticks, double amplifier, ModifierPriority priority, PotionEffectType effect, Material icon) {
        ...
        this.displayName = Utils.chat("&7&lAdd Base Effect: &e&" + effect.getName());
        this.icon = icon;
    }
    ...
    public ItemStack processItem(Player crafter, ItemStack outputItem) {
        ...
        PotionAttributesManager.getInstance().addDefaultStat(outputItem, new VanillaPotionEffectWrapper(effect.toString(), strength2, (long) Math.floor(strength)));
        return outputItem;
    }

    @Override
    public String toString() {
        return Utils.chat(String.format("&7Gives the potion &e%s %s for %s&7. Recipe is cancelled if item already has this potion effect.", effect.getName(), Utils.toRoman((int) Math.floor(strength2)), Utils.ticksToTimestamp((int) Math.floor(strength), 20)));
    }
}
