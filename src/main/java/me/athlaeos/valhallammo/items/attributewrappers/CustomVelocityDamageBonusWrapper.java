...
public class CustomVelocityDamageBonusWrapper extends AttributeWrapper{
    private final String displayLore = Utils.chat(TranslationManager.getInstance().getTranslation("translation_velocitydamagebonus"));

    /**
     * Velocity damage bonus adds damage to light or heavy weapons based on the velocity of the player. This damage multiplier
     * cannot go below 0, so even when moving away from the hit target the damage is not reduced.
     */
    public CustomVelocityDamageBonusWrapper(double amount, AttributeModifier.Operation operation, EquipmentSlot slot) {
        super(amount, operation, slot);
        this.attribute = "CUSTOM_VELOCITY_DAMAGE_BONUS";
