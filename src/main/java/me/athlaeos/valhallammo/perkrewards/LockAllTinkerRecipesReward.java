...
import me.athlaeos.valhallammo.crafting.dom.AbstractCraftingRecipe;
...
public class LockAllCraftRecipesReward extends PerkReward {
     ...
     */
    public LockAllCraftRecipesReward(String name, Object argument) {
        super(name, argument);
    }
    ...
    public void execute(Player player) {
        ...
        if (profile instanceof AccountProfile){
            ...
            unlockedRecipes.removeAll(CustomRecipeManager.getInstance().getAllRecipes().values().stream().filter(
                    abstractCraftingRecipe -> abstractCraftingRecipe instanceof ItemCraftingRecipe
            ).map(AbstractCraftingRecipe::getName).collect(Collectors.toList()));
