...
public class ArcheryProfile extends Profile implements Serializable {
    ...
    public void createProfileTable(Connection conn) throws SQLException {
        PreparedStatement stmt = conn.prepareStatement("CREATE TABLE IF NOT EXISTS profiles_archery (" +
    ...
    public void insertOrUpdateProfile(Connection conn) throws SQLException {
        PreparedStatement stmt = conn.prepareStatement(
                "REPLACE INTO profiles_archery " +
                        ...
                        "chargedshotdamagemultiplier, bowexpmultiplier, crossbowexpmultiplier, generalexpmultiplier) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
        ...
        stmt.setDouble(24, bowexpmultiplier);
        stmt.setDouble(25, crossbowexpmultiplier);
        stmt.setDouble(26, generalexpmultiplier);
        stmt.execute();
    }

    @Override
    public Profile fetchProfile(Player p, Connection conn) throws SQLException {
        PreparedStatement stmt = conn.prepareStatement("SELECT * FROM profiles_archery WHERE owner = ?;");
