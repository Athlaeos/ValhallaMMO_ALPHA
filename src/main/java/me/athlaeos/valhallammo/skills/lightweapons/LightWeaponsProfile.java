...
public class LightWeaponsProfile extends Profile implements Serializable {
    ...
    public void setDefaultStats(Player player) {
        ...
        if (skill != null){
            if (skill instanceof LightWeaponsSkill){
                LightWeaponsSkill lightWeaponsSkill = (LightWeaponsSkill) skill;
                for (PerkReward startingPerk : lightWeaponsSkill.getStartingPerks()){
    ...
    public void createProfileTable(DatabaseConnection conn) throws SQLException {
        PreparedStatement stmt = conn.getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS profiles_light_weapons (" +
    ...
    public void insertOrUpdateProfile(DatabaseConnection conn) throws SQLException {
        PreparedStatement stmt = conn.getConnection().prepareStatement(
                "REPLACE INTO profiles_light_weapons " +
                        ...
                        "stunchance, stunduration, dropbonus, raredropmultiplier, expmultiplier) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
        ...
        stmt.setFloat(36, stunchance);
        stmt.setInt(37, stunduration);
        stmt.setFloat(38, dropbonus);
        stmt.setFloat(39, raredropmultiplier);
        stmt.setDouble(40, expmultiplier);
        stmt.execute();
    }
